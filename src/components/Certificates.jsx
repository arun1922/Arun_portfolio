...
const CertificateCard = ({ cert, aosDelay }) => {
  const isAwsCert = cert.issuer === 'AWS Academy' || cert.name.includes('AWS');

  return (
    <div 
      data-aos="zoom-in"
      data-aos-delay={aosDelay}
      className="bg-black/20 backdrop-blur-sm rounded-2xl p-5 border border-white/10 hover:border-[#FF9900]/50 hover:scale-105 hover:shadow-[0_15px_40px_rgba(255,153,0,0.2)] transition-all duration-500 cursor-default group"
    >
      <div className="flex items-start gap-4">
        {isAwsCert ? (
          <div className="w-8 h-8 shrink-0 mt-0.5 p-0.5 rounded-lg bg-black/40 border border-[#FF9900]/30 group-hover:border-[#FF9900] group-hover:scale-110 transition-all duration-300 flex items-center justify-center">
        ...
        <div>
          ...
          <p className="text-white/50 text-xs font-semibold uppercase tracking-wider flex items-center gap-1.5">
            {isAwsCert && <span className="w-1.5 h-1.5 rounded-full bg-[#FF9900] inline-block animate-pulse"></span>}
            {cert.issuer}
          </p>
        </div>
      </div>
