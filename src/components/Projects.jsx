...
const ProjectCard = ({ project, aosDelay }) => (
  ...
  >
    <div className={`rounded-2xl p-6 md:p-8 h-full backdrop-blur-md transition-all duration-500 flex flex-col lg:flex-row gap-8 items-stretch ${
      project.isFlagship 
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2 mb-8">
            {project.techTags.map((tag) => {
              const isAwsTag = tag.toLowerCase().includes('aws') || tag.toLowerCase().includes('amazon');
              return (
                <span 
                  key={tag}
                  className={`px-3 py-1 text-xs font-bold rounded-full border transition-all duration-300 cursor-default flex items-center gap-1.5 ${
                    isAwsTag 
                      ? 'text-amber-200 bg-[#FF9900]/10 border-[#FF9900]/40 hover:bg-[#FF9900]/20 hover:border-[#FF9900]' 
                      : 'text-white/70 bg-white/5 border-white/10 hover:bg-red-500/20 hover:border-red-500/30 hover:text-red-300'
                  }`}
                >
                  {isAwsTag && (
                    <span className="w-3.5 h-3.5 inline-block shrink-0">
                      <AwsLogo textColor="#FFFFFF" arrowColor="#FF9900" className="w-full h-full" />
                    </span>
                  )}
                  {tag}
                </span>
      ...
      {project.image && (
        <div className="w-full lg:w-2/5 shrink-0 rounded-xl overflow-hidden border border-white/10 bg-black/40 group-hover:border-red-500/30 transition-all duration-500 flex items-center justify-center">
          <img 
            src={project.image} 
            alt={project.title} 
            className="w-full h-full object-cover max-h-[300px] lg:max-h-full group-hover:scale-105 transition-transform duration-700 ease-out"
          />
        </div>
      )}
    </div>
